...
@with_enricher_and_loading_bar(
    ...
    title="Getting list of users who liked Bluesky posts",
    unit="posts",
    nested=True,
    sub_unit="users",
)
def action(cli_args, enricher: Enricher, loading_bar: LoadingBar):
    ...
    for row, param in enricher.cells(cli_args.column, with_rows=True):
        ...
        with loading_bar.step(uri, sub_total=sub_total):
            for profile in islice(client.post_liked_by(uri), sub_total):
                liked_row = format_partial_profile_as_csv_row(profile)
                enricher.writerow(row, liked_row)
